...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(name=request.form["name"], email=form.email.data, password=password) #type: ignore
        db.session.add(new_user)

        try:
            db.session.commit()
        except sqlalchemy.exc.IntegrityError:
            flash("The email has been already used, log in instead!")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash("The user doesn't exist. Please Try Again")
            return redirect(url_for("login"))
